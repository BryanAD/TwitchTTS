...
namespace TextToSpeechTTV {
    class SpeechHelper {
        ...
        public void Speak_gcp (User user, string text, bool readOut) {
            ...
            int randPitch = r.Next(-10, 5);
            double randSpeed = r.NextDouble() * (1.25 - 0.5) + 0.5;
            Console.Write("User = " + user.Name + "; Name = " + randName + "; Pitch = " + randPitch + "; Speed = " + randSpeed + "\n");
            switch (userData.Voice.ToLower()) {
                case "random":
                    voiceName = voiceList[randName];
                    randomVoice = true;
                    googleSpeakingRate = randSpeed;
